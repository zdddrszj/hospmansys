...
<html>
  <head>
    ...
    <title>My JSP 'factoryModify.jsp' starting page</title>
	...
	function getFactoryCallBack(){
		...
		function factoryValidate(){
			/*1.创建XMLHttpRequest对象*/
				...
				//post方式
				//3.设置和服务器端交互的参数
				var factoryName = document.getElementById("factoryName").value;//新值
				var factoryId = document.getElementById("facId").value;//原Id
				xmlhttp.open("post","servlet/FactoryValidateServlet",false);
				xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");			
				//4.设置向服务器端发送的数据，启动和服务器端的交互
				xmlhttp.send("factoryName="+factoryName+"&factoryId="+factoryId);
	...
	<%
		Factory fac = (Factory)request.getAttribute("oneFactory");
	%>
  ...
  <body onload="getFactory();">
  	<center>
   		<form action="servlet/FactoryModifyServlet" method="post" onsubmit="return validate();">
   			<br />
   			<input type="hidden" name="facId" id="facId" value="<%=fac.getId() %>" />
   			<input type="hidden" name="proId" id="proId" value="<%=fac.getProvinceId() %>" />
   			<input type="hidden" name="cityId" id="cityId" value="<%=fac.getCityId() %>" />
   			<table>
   				<tr>
   					<td width="120px">药厂名称：</td>
   					<td width="200px"><input type="text" id="factoryName" name="factoryName" value="<%=fac.getName() %>" onfocus="factoryFocus();" onblur="validate();" /></td>
   					<td width="200px">
   				...
   				<tr>
   					<td>&nbsp;</td>
   					<td>&nbsp;</td>
   					<td align="left"><input type="submit" value="药厂修改"  /></td>
   					<td>&nbsp;</td>
   				</tr>
   			</table>
   		</form>
